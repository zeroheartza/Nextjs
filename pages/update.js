...
export default function update() {
    ...
    const onSubmit = async (body) => {
        ...
        console.log(body);
        const res = await axios.put(`http://localhost:8000/api/editUser/${body.id}`, {
        ...
        <div>
            ...
            <div class="container">
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <input type="name" placeholder="Enter name" class="form-control" {...register("id")} />

                    <label for="name">name:</label>
                    <input type="name" placeholder="Enter name" class="form-control" {...register("name")} />

                    <label for="email">Email:</label>
                    <input type="email" placeholder="Enter email" class="form-control" {...register("email")} />

                    <label for="password">password:</label>
                    <input type="password" placeholder="Enter password" class="form-control" {...register("password")} />
